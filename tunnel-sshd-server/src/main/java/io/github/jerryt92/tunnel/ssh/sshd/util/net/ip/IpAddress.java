...
public class IpAddress implements Comparable<IpAddress> {
    ...
    public static IpAddress valueOf(String value) {
        ...
        try {
            if (value.startsWith("[")) {
                inetAddress = InetAddresses.forUriString(value);
            } else {
    ...
    public boolean isMulticast() {
        if(isIp4()) {
            Ip4Address ip4 = this.getIp4Address();
            return (null != ip4 && Ip4Prefix.IPV4_MULTICAST_PREFIX.contains(ip4));
        } else {
            Ip6Address ip6 = this.getIp6Address();
            return (null != ip6 && Ip6Prefix.IPV6_MULTICAST_PREFIX.contains(ip6));
        }
    }
    ...
    public boolean isLinkLocal() {
        if(isIp4()) {
            Ip4Address ip4 = this.getIp4Address();
            return null != ip4 && Ip4Prefix.IPV4_LINK_LOCAL_PREFIX.contains(ip4);
        } else {
            Ip6Address ip6 = this.getIp6Address();
            return null != ip6 && Ip6Prefix.IPV6_LINK_LOCAL_PREFIX.contains(ip6);
        }
    }
    ...
    public int compareTo(IpAddress o) {
        ...
        for (int i = 0; i < this.octets.length; i++) {
            if (this.octets[i] != o.octets[i]) {
                return Byte.compareUnsigned(this.octets[i], o.octets[i]);
            }
        }
        return 0;       // Equal
    }
